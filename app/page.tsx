...
export default function SerpentineOdyssey() {
  ...
  const [currentLevelTargetScore, setCurrentLevelTargetScore] = useState(LEVEL_DATA[0].targetScore)
  const [foodEatenInLevel, setFoodEatenInLevel] = useState(0) // Track food eaten for speed increase
  const [unreachableFoodPendingShield, setUnreachableFoodPendingShield] = useState(false) // Track if an unreachable food was spawned
  ...
  const [unlockedTools, setUnlockedTools] = useState<Array<{ id: string; uses: number }>>([]) // Array of tool IDs with uses
  ...
  const [currentDailyChallenge, setCurrentDailyChallenge] = useState<DailyChallenge | null>(null)
  ...
  // Unlocked Themes
  const [unlockedThemes, setUnlockedThemes] = useState<string[]>(["aurora", "ocean", "sunset", "cosmic"]) // Default unlocked themes
  ...
  useEffect(() => {
    ...
    unlockNotificationSoundRef.current = new Audio("/sounds/unlock-notification.wav")
  ...
  const playSound = useCallback(
    (
      ...
      if (audioRef.current) {
        ...
        audioRef.current.play().catch((e) => console.error(`Audio playback failed for ${soundType}:`, e))
      }
    },
  ...
  useEffect(() => {
    const achievementList: Achievement[] = [
      ...
      },
      {
        ...
        icon: "📈",
        unlocked: false,
        condition: (stats) => stats.levelsCleared > 0 && stats.shrinkPotionsEaten === 0, // New condition
    ...
    setSessionAchievements(achievementList.map((a) => ({ ...a, unlocked: false })))
  }, [settings.difficulty])
  ...
  const getWeightedFoodTypes = useCallback(
    (difficulty: GameSettings["difficulty"]) => {
      const basePointFoodTypes = [
        { type: "apple", points: 10, color: "#ef4444", emoji: "🍎", weight: 40, shape: "circle" },
        { type: "cherry", points: 20, color: "#ec4899", emoji: "🍒", weight: 20, shape: "circle" },
        { type: "gem", points: 50, color: "#06b6d4", emoji: "💎", weight: 10, shape: "diamond" },
      ]
      ...
      let adjustedSpecialFoodTypes = baseSpecialFoodTypes.map((food) => ({ ...food }))

      if (difficulty === "easy") {
        adjustedSpecialFoodTypes = adjustedSpecialFoodTypes.map((food) => {
          if (["speed", "multiplier", "shield", "growth", "teleport", "life-potion"].includes(food.type)) {
            return { ...food, weight: food.weight * 1.5 } // More common
          }
          if (["slow", "shrink", "fragile-walls", "poison"].includes(food.type)) {
      ...
      return { pointFoodTypes: basePointFoodTypes, specialFoodTypes: adjustedSpecialFoodTypes }
    },
    [currentTheme],
  )
  ...
  const generateFood = useCallback(
    (isSpecialSpawn = false): Food => {
      const { pointFoodTypes, specialFoodTypes } = getWeightedFoodTypes(settings.difficulty)
      ...
      let selectedType: (typeof pointFoodTypes)[number] | (typeof specialFoodTypes)[number]
      ...
      if (isSpecialSpawn) {
        ...
        if (isPoisoned || isWallFragile) {
          filteredSpecialTypes = filteredSpecialTypes.filter((t) => t.type === "shield")
          if (filteredSpecialTypes.length === 0) {
            filteredSpecialTypes = specialFoodTypes.filter((t) => t.type === "shield") // Fallback
          }
        } else if (activeBuffs.some((b) => b.type === "slow")) {
          // Prioritize speed if slow debuff is active
          filteredSpecialTypes = filteredSpecialTypes.filter((t) => t.type === "speed")
          if (filteredSpecialTypes.length === 0) {
            filteredSpecialTypes = specialFoodTypes.filter((t) => t.type === "speed") // Fallback
          }
        } else if (activeBuffs.some((b) => b.type === "speed")) {
          // Prioritize slow if speed buff is active (to balance)
          filteredSpecialTypes = filteredSpecialTypes.filter((t) => t.type === "slow")
          if (filteredSpecialTypes.length === 0) {
            filteredSpecialTypes = specialFoodTypes.filter((t) => t.type === "slow") // Fallback
          }
        }
        ...
        const totalWeight = filteredSpecialTypes.reduce((sum, type) => sum + type.weight, 0)
        ...
        const totalWeight = pointFoodTypes.reduce((sum, type) => sum + type.weight, 0)
        let random = Math.random() * totalWeight
        selectedType =
      ...
      if (unreachableFoodPendingShield) {
        selectedType = specialFoodTypes.find((t) => t.type === "shield") || pointFoodTypes[0] // Ensure shield is selected
      ...
      while (!isValidPosition && attempts < MAX_ATTEMPTS) {
        ...
        const onBotSnake = gameMode === "coop" && botSnake.some((s) => s.x === position.x && s.y === position.y)
      ...
      const isTimedFood = ["speed", "multiplier", "slow", "shield", "shrink", "fragile-walls", "poison"].includes(
        selectedType.type,
      )
  ...
  const createParticles = useCallback(
    (x: number, y: number, particleType: Food["type"] | "disappear" | "wall-break") => {
      let color: string
      let count: number
      let size: number
    ...
    (toolId: string | null) => {
      ...
      const addOrUpdateBuff = (newBuff: (typeof activeBuffs)[0]) => {
        setActiveBuffs((prevBuffs) => {
          const existingBuffIndex = prevBuffs.findIndex((b) => b.type === newBuff.type)
          if (existingBuffIndex !== -1) {
            // If buff exists, extend its duration
            return prevBuffs.map((b, i) => (i === existingBuffIndex ? { ...b, timeLeft: newBuff.timeLeft } : b))
          }
  ...
  const initializeGameSession = useCallback(
    (mode: GameMode, startLevelIndex = 0) => {
      ...
      let initialSnakeHead: Position = { x: Math.floor(maxX / 2), y: Math.floor(maxY / 2) }
      let attempts = 0
      const MAX_SPAWN_ATTEMPTS = 100

      // Calculate obstacle offset for centering
      const levelObstacles = mode === "levels" ? LEVEL_DATA[startLevelIndex].obstacles : [] // No fixed obstacles in endless/coop
      const offsetX = Math.floor((maxX - GRID_COLS) / 2)
      const offsetY = Math.floor((maxY - GRID_ROWS) / 2)
      const currentObstacles = levelObstacles.map((o) => ({ x: o.x + offsetX, y: o.y + offsetY }))
      setObstacles(currentObstacles)

      // Ensure snake spawns in a clear spot
      while (
        currentObstacles.some((o) => o.x === initialSnakeHead.x && o.y === initialSnakeHead.y) &&
      ...
      if (attempts === MAX_SPAWN_ATTEMPTS) {
        console.warn("Could not find a clear spawn point for snake, placing at default.")
      ...
      setCurrentLevelTargetScore(mode === "levels" ? LEVEL_DATA[startLevelIndex].targetScore : Number.POSITIVE_INFINITY) // Set target score or Infinity for endless
      ...
      if (mode === "coop") {
        let initialBotHead: Position = { x: Math.floor(maxX / 2) + 5, y: Math.floor(maxY / 2) } // Spawn bot away from player
        attempts = 0
        while (
          (currentObstacles.some((o) => o.x === initialBotHead.x && o.y === initialBotHead.y) ||
            (initialBotHead.x === initialSnakeHead.x && initialBotHead.y === initialSnakeHead.y)) &&
          attempts < MAX_SPAWN_ATTEMPTS
        ) {
          initialBotHead = {
            x: Math.floor(Math.random() * maxX),
      ...
      if (equippedTool) {
        ...
        const equippedToolInState = unlockedTools.find((t) => t.id === equippedTool)

        if (toolToApply && equippedToolInState && equippedToolInState.uses > 0) {
          ...
          setUnlockedTools((prev) => {
            ...
            const toolAfterUse = updatedTools.find((t) => t.id === equippedTool)
      ...
      const hasBeneficialFood = initialFood.some((f) => !["slow", "shrink", "fragile-walls", "poison"].includes(f.type))
    ...
    (toolId: string) => {
      ...
      if (existingTool) {
        if (tool.stackable && currency >= tool.cost) {
          setCurrency((prev) => prev - tool.cost)
          setUnlockedTools((prev) =>
            prev.map((t) => (t.id === tool.id ? { ...t, uses: t.uses + tool.initialUses } : t)),
          )
          playSound("powerup")
          return true
        }
        ...
        if (currency >= tool.cost) {
          setCurrency((prev) => prev - tool.cost)
          setUnlockedTools((prev) => [...prev, { id: tool.id, uses: tool.initialUses }])
          playSound("powerup")
          return true
        }
      }
      ...
      if (currentPage === "game") {
        ...
        if (gameState === "playing") {
          ...
          if (newDirection) {
            ...
            const currentDir =
              directionQueue.current.length > 0 ? directionQueue.current[directionQueue.current.length - 1] : direction
  ...
  }, [currentPage, gameState, settings.controlScheme, pauseGame, navigateToPage, direction]) // Added direction to dependencies for input queue logic
  ...
  useEffect(() => {
    ...
    const savedLastBeatenLevel = localStorage.getItem("serpentineLastBeatenLevel")
    const savedLastLifeRegenTime = localStorage.getItem("serpentineLastLifeRegenTime")
    const hasPlayedBefore = localStorage.getItem("hasPlayedBefore")
    const savedUnlockedThemes = localStorage.getItem("serpentineUnlockedThemes")
    const savedDailyChallenge = localStorage.getItem("serpentineDailyChallenge")
    const savedIsDailyChallengeCompleted = localStorage.getItem("serpentineIsDailyChallengeCompleted")
    const savedLastDailyChallengeDate = localStorage.getItem("serpentineLastDailyChallengeDate")
    ...
    if (savedOverallStats) {
      ...
      const defaultFoodEatenByType = Object.keys(overallStats.foodEatenByType).reduce(
        (acc, type) => {
      ...
      setOverallStats({
        ...
        ...parsedStats,
        foodEatenByType: { ...defaultFoodEatenByType, ...parsedStats.foodEatenByType },
      })
    }
    ...
    if (savedLastBeatenLevel) setLastBeatenLevel(Number.parseInt(savedLastBeatenLevel))

    // Load lives and apply regeneration
    let loadedLives = savedLives ? Number.parseInt(savedLives) : MAX_LIVES
    const loadedLastRegenTime = savedLastLifeRegenTime ? Number.parseInt(savedLastLifeRegenTime) : Date.now()

    const now = Date.now()
    const timeElapsed = now - loadedLastRegenTime
    const livesToRegen = Math.floor(timeElapsed / (LIVES_REGEN_INTERVAL_SECONDS * 1000))

    loadedLives = Math.min(MAX_LIVES, loadedLives + livesToRegen * LIVES_REGEN_AMOUNT)
    setLives(loadedLives)
    lastLifeRegenTimeRef.current = now - (timeElapsed % (LIVES_REGEN_INTERVAL_SECONDS * 1000)) // Update last regen time to account for partial interval

    if (savedUnlockedThemes) setUnlockedThemes(JSON.parse(savedUnlockedThemes))

    // Load daily challenge state
    if (savedDailyChallenge) setCurrentDailyChallenge(JSON.parse(savedDailyChallenge))
    if (savedIsDailyChallengeCompleted) setIsDailyChallengeCompleted(JSON.parse(savedIsDailyChallengeCompleted))
    if (savedLastDailyChallengeDate) setLastDailyChallengeDate(savedLastDailyChallengeDate)
  ...
  useEffect(() => {
    ...
    localStorage.setItem("serpentineUnlockedTools", JSON.stringify(unlockedTools))
    localStorage.setItem("serpentineEquippedTool", equippedTool || "")
    localStorage.setItem("serpentineLives", lives.toString())
    localStorage.setItem("serpentineLastBeatenLevel", lastBeatenLevel.toString())
    localStorage.setItem("serpentineLastLifeRegenTime", lastLifeRegenTimeRef.current.toString())
    localStorage.setItem("serpentineOverallStats", JSON.stringify(overallStats)) // Save overall stats
    localStorage.setItem("serpentineUnlockedThemes", JSON.stringify(unlockedThemes))
    localStorage.setItem("serpentineDailyChallenge", JSON.stringify(currentDailyChallenge))
    localStorage.setItem("serpentineIsDailyChallengeCompleted", JSON.stringify(isDailyChallengeCompleted))
    localStorage.setItem("serpentineLastDailyChallengeDate", lastDailyChallengeDate)
  }, [
    settings,
    currency,
    unlockedTools,
  ...
  const checkAchievements = useCallback(() => {
    setOverallAchievements((prevOverall) => {
      ...
      prevOverall.forEach((achievement) => {
        if (!achievement.unlocked && achievement.condition(overallStats)) {
          updatedOverall = updatedOverall.map((ach) => (ach.id === achievement.id ? { ...ach, unlocked: true } : ach))
          ...
          setOverallStats((prev) => ({
            ...prev,
            totalCurrencyEarned: prev.totalCurrencyEarned + ACHIEVEMENT_CURRENCY_REWARD,
          }))
          newAchievementUnlocked = true

          if (achievement.toolRewardId) {
            const toolToReward = TOOLS_DATA.find((t) => t.id === achievement.toolRewardId)
            if (toolToReward) {
              setUnlockedTools((prevUnlocked) => {
                const existingTool = prevUnlocked.find((t) => t.id === toolToReward.id)
                if (existingTool) {
                  if (toolToReward.stackable) {
                    return prevUnlocked.map((t) =>
                      t.id === toolToReward.id ? { ...t, uses: t.uses + toolToReward.initialUses } : t,
                    )
                  }
                  return prevUnlocked
                } else {
                  return [...prevUnlocked, { id: toolToReward.id, uses: toolToReward.initialUses }]
                }
              })
            }
          }
      ...
      if (newAchievementUnlocked) {
        ...
        const unlockedIds = updatedOverall.filter((a) => a.unlocked).map((a) => a.id)
    ...
    setSessionAchievements((prevSession) => {
      ...
      prevSession.forEach((achievement) => {
        if (!achievement.unlocked && achievement.condition(overallStats)) {
          updatedSession = updatedSession.map((ach) => (ach.id === achievement.id ? { ...ach, unlocked: true } : ach))
        }
      })
    ...
    if (currentDailyChallenge && !isDailyChallengeCompleted && currentDailyChallenge.condition(overallStats)) {
      setIsDailyChallengeCompleted(true)
      playSound("achievement") // Use achievement sound for daily challenge completion
      if (currentDailyChallenge.rewardType === "currency" && currentDailyChallenge.rewardAmount) {
        setCurrency((prev) => prev + currentDailyChallenge.rewardAmount!)
        setOverallStats((prev) => ({
          ...prev,
          totalCurrencyEarned: prev.totalCurrencyEarned + currentDailyChallenge.rewardAmount!,
        }))
        console.log(`Daily challenge completed! Gained ${currentDailyChallenge.rewardAmount} currency.`)
      } else if (currentDailyChallenge.rewardType === "theme" && currentDailyChallenge.rewardId) {
        setUnlockedThemes((prev) => {
          if (!prev.includes(currentDailyChallenge.rewardId!)) {
            console.log(`Daily challenge completed! Unlocked theme: ${currentDailyChallenge.rewardId}.`)
  ...
  const getBotDirection = useCallback(
    (botHead: Position, botBody: Position[], foodItems: Food[], currentObstacles: Position[]): Position => {
      const maxX = Math.floor(canvasWidth / GRID_SIZE)
      const maxY = Math.floor(canvasHeight / GRID_SIZE)
      ...
      if (closestFood) {
        ...
        else if (targetX < botHead.x) possibleDirections.unshift({ x: -1, y: 0 })

        // Then vertical movement
        if (targetY > botHead.y) possibleDirections.unshift({ x: 0, y: 1 })
        else if (targetY < botHead.y) possibleDirections.unshift({ x: 0, y: -1 })
      }
      ...
      const safeDirections = possibleDirections.filter((dir) => {
        ...
        const collisionWithSelf = botBody.some((segment, idx) => idx > 0 && segment.x === nextX && segment.y === nextY)
        const collisionWithObstacle = currentObstacles.some((o) => o.x === nextX && o.y === nextY)
        const collisionWithPlayer = snake.some((s) => s.x === nextX && s.y === nextY)

        return !collisionWithSelf && !collisionWithObstacle && !collisionWithPlayer
      })
      ...
      if (
        !(
          botBody.some(
            (segment, idx) =>
              idx > 0 && segment.x === botHead.x + reverseDir.x && segment.y === botHead.y + reverseDir.y,
          ) ||
          currentObstacles.some((o) => o.x === botHead.x + reverseDir.x && o.y === botHead.y + reverseDir.y) ||
          snake.some((s) => s.x === botHead.x + reverseDir.x && s.y === botHead.y + reverseDir.y)
        )
      ) {
        return reverseDir
      }
    ...
    const gameLoop = (currentTime: number) => {
      if (currentTime - lastTimeRef.current >= speed) {
        ...
        setSnake((prevSnake) => {
          ...
          const isInvincible = activeBuffs.some((buff) => buff.type === "shield")
          const isTempSelfInvincible = activeBuffs.some((b) => b.type === "temp-self-invincible")
          ...
          if (head.x < 0 || head.x >= maxX || head.y < 0 || head.y >= maxY) {
            if (isWallFragile) {
              ...
              setOverallStats((prev) => ({ ...prev, obstaclesBroken: prev.obstaclesBroken + 1 }))
            } else {
          ...
          // Obstacle collision
          let obstacleCollision = obstacles.some((obstacle) => obstacle.x === head.x && obstacle.y === head.y)
          // Bot collision
          const botCollision =
            gameMode === "coop" && botSnake.some((segment) => segment.x === head.x && segment.y === head.y)

          if (obstacleCollision && isWallBreakerActive) {
            // If wall breaker is active, consume a use and remove the obstacle
            setWallBreakerUses((prev) => prev - 1)
            setObstacles((prev) => prev.filter((o) => !(o.x === head.x && o.y === head.y)))
            playSound("general_positive_effect")
          ...
          if (
            (selfCollision || obstacleCollision || wallCollision || botCollision) &&
            ...
            setOverallStats((prev) => {
              const newStats = {
                ...
                totalPlayTime: prev.totalPlayTime + Math.floor((Date.now() - gameStartTimeRef.current) / 1000), // Add session play time
              }
          ...
          if (eatenFoodIndex !== -1) {
            ...
            const points = eatenFood.points * (activeBuffs.some((buff) => buff.type === "multiplier") ? 2 : 1)

            setScore((prev) => prev + points) // Update cumulative score
            setCurrency(
              (prev) =>
                prev + Math.floor(points / 5) + 1 * (activeBuffs.some((b) => b.type === "coin-booster-active") ? 2 : 1),
            ...
            setOverallStats((prev) => ({
              ...
              foodEatenByType: {
                ...
                [eatenFood.type]: (prev.foodEatenByType[eatenFood.type] || 0) + 1,
              }, // Track food by type
              maxLength: Math.max(prev.maxLength, newSnake.length),
              shrinkPotionsEaten: eatenFood.type === "shrink" ? prev.shrinkPotionsEaten + 1 : prev.shrinkPotionsEaten,
              totalCurrencyEarned:
                prev.totalCurrencyEarned +
                Math.floor(points / 5) +
                1 * (activeBuffs.some((b) => b.type === "coin-booster-active") ? 2 : 1),
            }))
            ...
            setActiveBuffs((updatedBuffs) => {
              ...
              const addOrUpdateBuff = (newBuff: (typeof activeBuffs)[0]) => {
                const existingBuffIndex = currentBuffs.findIndex((b) => b.type === newBuff.type)
                if (existingBuffIndex !== -1) {
                  // If buff exists, extend its duration
              ...
              if (eatenFood.type === "shield") {
                currentBuffs = currentBuffs.filter((b) => {
                  ...
                  if (b.type === "poisoned") {
                    setIsPoisoned(false)
                    setOverallStats((prev) => ({ ...prev, poisonCuredCount: prev.poisonCuredCount + 1 }))
                    playSound("general_positive_effect")
                    return false
                  }
                  return true
                ...
                setOverallStats((prev) => ({ ...prev, totalCurrencyEarned: prev.totalCurrencyEarned + 10 }))
                addOrUpdateBuff({
                  type: "fragile-walls",
                ...
                setFood((prev) =>
                  prev.map((f) => (f.timeLeft !== undefined ? { ...f, timeLeft: Math.max(1, f.timeLeft * 0.8) } : f)),
                )
                addOrUpdateBuff({
                  type: "speed",
                  timeLeft: getEffectDuration(true, 300),
                ...
                setFood((prev) =>
                  prev.map((f) => (f.timeLeft !== undefined ? { ...f, timeLeft: Math.min(600, f.timeLeft * 1.2) } : f)),
                )
                addOrUpdateBuff({
                  type: "slow",
                  timeLeft: getEffectDuration(false, 300),
                ...
                setDirection(randomDirections[Math.floor(Math.random() * randomDirections.length)])
                ...
                while (!newPosition && teleportAttempts < maxTeleportAttempts) {
                  ...
                  const onSnake = newSnake.some((s) => s.x === potentialPos.x && s.y === potentialPos.y)
                  const onObstacle = obstacles.some((o) => o.x === potentialPos.x && o.y === potentialPos.y)
                  const onBotSnake =
                    gameMode === "coop" && botSnake.some((s) => s.x === potentialPos.x && s.y === potentialPos.y)
            ...
            const isLengthLocked = activeBuffs.some((b) => b.type === "length-lock")
            if (!isLengthLocked) {
              newSnake.pop()
            }
          }
        ...
        if (gameMode === "coop" && botActiveRef.current) {
          setBotSnake((prevBotSnake) => {
            ...
            const newBotDir = getBotDirection(botHead, newBotSnake, food, obstacles)
            ...
            const botSelfCollision = newBotSnake
              .slice(1)
              .some((segment) => segment.x === botHead.x && segment.y === botHead.y)
            const botObstacleCollision = obstacles.some(
              (obstacle) => obstacle.x === botHead.x && obstacle.y === botHead.y,
            )
            const botPlayerCollision = snake.some((segment) => segment.x === botHead.x && segment.y === botHead.y)

            if (botSelfCollision || botObstacleCollision || botPlayerCollision) {
            ...
            const botEatenFoodIndex = food.findIndex((f) => f.position.x === botHead.x && f.position.y === botHead.y)
        ...
        if (gameMode === "levels" && level < LEVEL_DATA.length - 1 && score >= currentLevelTargetScore) {
          const nextLevel = level + 1
          setOverallStats((prev) => ({ ...prev, levelsCleared: prev.levelsCleared + 1 })) // Update levels cleared
        ...
        const isLengthLocked = activeBuffs.some((b) => b.type === "length-lock")
        if (isPoisoned && snake.length > 1 && lastTimeRef.current % 60 === 0 && !isLengthLocked) {
          // Shrink every second
          setSnake((prev) => {
          ...
          const updatedFood = prevFood
            ...
            .filter((f) => {
              if (f.timeLeft !== undefined && f.timeLeft <= 0) {
                ...
                setFood((currentFood) => [...currentFood, generateFood(Math.random() < 0.3)]) // 30% chance for special food
        ...
        const isFoodAttractorActive = activeBuffs.some((b) => b.type === "food-attractor-active")
      ...
      setFireworks((prev) =>
        prev
          ...
          .filter((firework: any) => firework.life > 0 && firework.particles.length > 0),
      )
  ...
  useEffect(() => {
    if (canvasWidth > 0 && canvasHeight > 0 && food.length === 0) {
      const difficultySettings = DIFFICULTY_SETTINGS[settings.difficulty]
      const initialFood: Food[] = []
      const foodCount = gameMode === "endless" ? difficultySettings.foodCount + 1 : difficultySettings.foodCount // More food in endless
  ...
  }, [canvasWidth, canvasHeight, food.length, generateFood, settings.difficulty, gameMode])
    ...
    <>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {currentPage === "help" && <HelpPage navigateToPage={navigateToPage as any} currentTheme={currentTheme} />}
          {currentPage === "levelSelect" && (
            <LevelSelectPage
              navigateToPage={navigateToPage as any}
              startGameSessionFromLevel={(levelIndex) => startGameSession("levels", levelIndex)} // Pass mode
          ...
          {currentPage === "levelStart" && (
            <LevelStartPage
              navigateToPage={navigateToPage as any}
              startGameSession={() => startGameSession("levels", level)} // Start current level in levels mode
      ...
      {showWelcomeModal && <WelcomeModal navigateToPage={navigateToPage as any} onClose={() => setShowWelcomeModal(false)} />}
    </>
  )
}
